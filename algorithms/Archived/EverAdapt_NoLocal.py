...
class EverAdapt_NoLocal(BaseAlgo):
    ...
    def epoch_train(self, src_loader, trg_loader, epoch, device):
        ...
        for step, (data) in enumerate(combined_loader):
            ...
            alpha = 0.9 ** epoch
            loss = alpha * self.hparams.src_cls_loss_wt * coral_loss + self.hparams.src_cls_loss_wt * classification_loss 
            # alpha = self.hparams.alpha #! Modified
            # loss = alpha * coral_loss + (1-alpha) * classification_loss 
