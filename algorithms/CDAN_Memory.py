...
class CDAN_Memory(BaseAlgo):
    ...
    def epoch_train(self, src_loader, trg_loader, epoch, device):
        ...
        for step, (data) in enumerate(combined_loader):
            ...
            # Added Memory
            if memory:
                domain_label_mem = torch.zeros(len(mem_x)).to(device)
                mem_label_concat = torch.cat((domain_label_src, domain_label_mem), 0).long()

                # mem features and predictions
                mem_feat = self.feature_extractor(mem_x)
                mem_pred = self.classifier(mem_feat)
                # concatenate features and predictions
                mem_feat_concat = torch.cat((src_feat, mem_feat), dim=0)
                mem_pred_concat = torch.cat((src_pred, mem_pred), dim=0)

                # Domain classification loss
                mem_feat_x_pred = torch.bmm(mem_pred_concat.unsqueeze(2), mem_feat_concat.unsqueeze(1)).detach()
                mem_disc_prediction = self.domain_classifier(mem_feat_x_pred.view(-1, pred_concat.size(1) * mem_feat_concat.size(1)))
                mem_disc_loss = self.taskloss(mem_disc_prediction, mem_label_concat)

                # update Domain classification
                self.domain_classifier_optimiser.zero_grad()
                mem_disc_loss.backward()
                self.domain_classifier_optimiser.step()

                # prepare fake domain labels for training the feature extractor
                domain_label_src = torch.zeros(len(src_x)).long().to(device)
                domain_label_mem = torch.ones(len(mem_x)).long().to(device)
                mem_label_concat = torch.cat((domain_label_src, domain_label_mem), 0)

                # Repeat predictions after updating discriminator
                mem_feat_x_pred = torch.bmm(pred_concat.unsqueeze(2), mem_feat_concat.unsqueeze(1))
                mem_disc_prediction = self.domain_classifier(mem_feat_x_pred.view(-1, pred_concat.size(1) * mem_feat_concat.size(1)))
                # loss of domain discriminator according to fake labels

                domain_loss += self.taskloss(mem_disc_prediction, mem_label_concat)

                # conditional entropy loss.
                loss_trg_cent += self.criterion_cond(mem_pred)
